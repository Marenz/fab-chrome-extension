...
async function readBinaryBody(body, headerVersion, signal) {
  ...
  if (reader.remaining() >= 4) {
    ...
    // Newer custom-field versions (e.g. 3, seen on UE 5.8-era Fab builds) may
    // change the section's internal layout. The parsed fields are never used
    // for downloading, and the section is size-prefixed, so skip it wholesale
    // for any version we don't know -- same strategy legendary uses.
...
function validateExpectedChunk(expected, inputLength, header) {
  ...
  if (expected.fileSize !== undefined && expected.fileSize !== null && expected.fileSize !== 0) {
    ...
    // Do not require inputLength === fileSize: on newer (UE 5.8-era) builds the
    // CDN may serve a chunk stored differently (e.g. uncompressed) than the
    // manifest's recorded fileSize. Integrity is guaranteed by the GUID,
    // rolling-hash and SHA-1 checks plus the decoded window-size check.
  }
